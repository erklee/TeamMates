    ...
    <div className="userEventsIndexItemContainter">
      <div className="userEventsItemWrapper">
        <div className="userEventsImageContainer">
          <div className="userEventsImageOverlayText" onClick={handleEventShow}>
            Go To Event
          </div>
          <img
            ...
            height={400}
            width={400}
            className="userEventsItemPic"
            onClick={handleEventShow}
          />
        </div>
        {isCoordinator && (
          <button className="userEventsItemEditButton" onClick={handleEdit}>
            Edit
          </button>
        )}
        <div className="userEventItemInfoWrapper">
          <h1 className="userEventItemTitle" onClick={handleEventShow}>
            {capitalizeEveryWord(event?.title)}
          </h1>
          <div className="userEventItemInfo">
            ...
            <p>
              ...
              {`${event?.category
                ?.slice(0, 1)
                .toUpperCase()}${event?.category?.slice(1)}`}
            </p>
            <p>
              <span>Difficulty:</span>{" "}
              {`${event?.difficulty
                ?.slice(0, 1)
                .toUpperCase()}${event?.difficulty?.slice(1)}`}
            </p>
            <p>
              <span>Address:</span>{" "}
            ...
            <p>
              <span>Participants:</span> {`${event.attendees?.length}`}
            </p>
            <a
              target="_blank"
