...
import allSports from "../../assets/images/sports/allSports2.jpeg";
...
function EventShowPage() {
  ...
  if (!event) {
    ...
    const attendeesList = event.attendees?.map((attendee, index) => {
      ...
      return (
        <li
          className="eventShowAttendee"
          onClick={handleUserProfile}
          key={index}
        >
          {attendee.fname?.slice(0, 1).toUpperCase() +
            attendee.fname?.slice(1) +
            " " +
            attendee.lname?.slice(0, 1).toUpperCase() +
            attendee.lname?.slice(1)}
        </li>
      );
    });
        ...
        <div className="eventShowPageWrapper">
          <div className="show-page-container">
            <div className="event-wrapper-info">
              <img
                className="eventShowPic"
                src={event.pictureUrl || allSports}
                alt="eventPicture"
                height={300}
                width={300}
              />
              ...
              {isUserAttending && currentUser ? (
                <button className="unattendBtn" onClick={handleUnattendClick}>
                  Unattend
                </button>
              ) : (
                <button
                  ...
                  type="submit"
                  disabled={event.attendeesMax === event.attendees.length}
                  style={{
                    backgroundColor:
                      event.attendeesMax === event.attendees.length
                        ? "gray"
                        : "rgb(255, 85, 0)",
                    color:
                      event.attendeesMax === event.attendees.length
              ...
              <div className="eventShowInfoWrapper">
                <div className="eventShowDetails">
                  ...
                  <p>
                    <span>Difficulty:</span> {event.difficulty}
                  </p>
                  <p>
                    <span>Coordinater:</span>{" "}
                    {`${event.coordinator.fname} ${event.coordinator.lname}`}
                  </p>
                  <p>
                    <span>Address:</span>{" "}
                  ...
                  <p>
                    <span>Max Capacity:</span> {`${event?.attendeesMax}`}{" "}
                  </p>
                  <a
                    target="_blank"
                    href={`https://maps.google.com/?q=${event.location.address}, ${event.location.zipcode}`}
                    rel="noreferrer"
                  >
                    Directions
                  </a>
